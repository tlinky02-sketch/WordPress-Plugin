...
const PlatformDetailedRow: React.FC<PlatformDetailedRowProps> = ({
    ...
    return (
        <div
            className={`group relative grid grid-cols-1 md:grid-cols-12 gap-6 p-6 rounded-xl border bg-card text-card-foreground shadow-sm transition-all hover:shadow-md items-center ${isFloating ? '' : 'overflow-hidden'}`}
            style={{
                ...
                {item.show_coupon && item.coupon_code && (
                    <div className="w-full">
                        <button
                            ...
                            className="w-full py-1 text-xs font-bold text-primary border border-dashed border-primary/50 rounded-lg bg-primary/5 hover:bg-primary/10 transition-colors flex items-center justify-center gap-2 cursor-pointer transition-all duration-200 relative z-20"
                            style={{
